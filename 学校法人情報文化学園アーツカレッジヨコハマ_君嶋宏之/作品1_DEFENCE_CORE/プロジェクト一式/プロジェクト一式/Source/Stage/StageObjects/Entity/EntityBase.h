...
static const float DEFAULT_ENTITY_SIZE = 100.0f * 2.5f; // ブロックの直径
...
/// エンティティの基底クラス
/// </summary>
class EntityBase : public StageObjectBase
{
public:
	/// <summary>
	///									コンストラクタ
	...
	EntityBase(const Transform& _trans, const ModelData& _modelData, const StageObjectData::STAGE_OBJECT_KIND& _kind, const int& _hp,const PUT_PLACE_KIND& _putPlaceKind = PUT_PLACE_KIND::PUT_POINT);
	virtual ~EntityBase();
...
protected:
	...
	virtual bool HitEntity(const CollisionHitInfoData& _targetData) { return false; }

	bool active;					// アクティブ状態かのフラグ
