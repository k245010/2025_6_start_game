...
namespace DxLib {
	...
	static const VECTOR VMax	= { FLT_MAX,FLT_MAX,FLT_MAX };
	static const VECTOR VOne	= { 1,1,1 };
	static const VECTOR VNormX	= { 1,0,0 };
	static const VECTOR VNormY	= { 0,1,0 };
	static const VECTOR VNormZ	= { 0,0,1 };
	...
	public:
		...
		/// <summary>
		/// クリップボードに自身のVECTOR3の値を設定する
		/// </summary>
		void SetClipboardTextVec()
		{
			char copyVec[CHAR_MAX]	= {};
			sprintfDx(copyVec, "%.2lf\t%.2lf\t%.2lf", x, y, z);
			std::string copyStr		= copyVec;
			SetClipboardText(copyStr.c_str());
		}
	};
	...
	struct VECTOR2 {
		...
		/// <summary>
		/// クリップボードに自身のVECTOR2の値を設定する
		/// </summary>
		void SetClipboardTextVec()
		{
			char copyVec[CHAR_MAX]	= {};
			sprintfDx(copyVec, "%.2lf,%.2lf", x, y);
			std::string copyStr		= copyVec;
			SetClipboardText(copyStr.c_str());
		}
	};
	...
	{
		_json.at("X").get_to(_vector.x);
		_json.at("Y").get_to(_vector.y);
		_json.at("Z").get_to(_vector.z);
	}
};
